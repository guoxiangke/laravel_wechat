...
class WechatJSSDK
{
  ...
  private function getAccessToken() {
    ...
    if (!$access_token) {
      ...
      if (is_object($res) && isset($res->access_token)) {
        ...
        \Log::error(__CLASS__,[__FUNCTION__,__LINE__,'no $access_token']);
      }
    }
    return $access_token;
  }
  ...
  private function get_cache($cacheKey) {
    return Cache::tags('wxjssdk')->get($cacheKey,false);
  }

  private function set_cache($cacheKey,$cacheValue) {
    Cache::tags('wxjssdk')->put($cacheKey, $cacheValue, now()->addMinutes(120));
  }
}
